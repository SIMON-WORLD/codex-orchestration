...
// Loads the dedicated study, probes the REAL runtime, runs the real resolver/preflight, then executes the
// active Data -> Empirical DAG through the actual tested implementations. Produces a fresh integrated
// scientific artifact bundle, runs the four narrow presentation renderers, validates provenance, and
// compares fresh vs frozen benchmark evidence. Supports --stage data|empirical|all for worker split.
...
function probeEnv() {
  ...
  try { const log = readFileSync(logPath, "utf8"); const m = log.match(/V=(\S+)/); if (m) stataLine = m[1]; if (log.includes("rg=INSTALLED")) reghdfeOk = true; } catch {}
...
function resolveStudy(study, envObj) { const registry = loadRegistry(join(root, "domains/economics/capabilities")); const ctx = { mode: study.execution_context.mode, allow_experimental: !!study.execution_context.allow_experimental, preferred_runtimes: study.execution_context.preferred_runtimes || [], approved_overrides: study.execution_context.approved_overrides || [] }; return { registry, preflight: resolveAll(study, registry, envObj, ctx) }; }
...
export async function runPhase1(stage = STAGE, outDir = OUT_DIR) {
  ...
  const { preflight } = resolveStudy(study, envObj);
