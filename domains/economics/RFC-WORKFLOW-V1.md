...
| data | `economics.data.validation` (Data Validation Pack v1: machine-checkable structural checks — declared-key duplicate detection, selected-variable missingness, row-count/observation-count consistency, required-variable presence + declared type consistency, sample-flow arithmetic, merge cardinality ONLY when a merge + expected relationship is explicitly declared), (+ future `.harmonize`, `.construct`) | **E1** `src-dime-data-handbook`, `src-dime-standards`; **E2** real-data Grunfeld + adversarial defect-detection. `data.val.python.pandas` & `data.val.stata` are `tested` (NOT verified) for the exact demonstrated structural checks; does NOT infer ranges/units, does NOT decide sample exclusion, does NOT auto-repair/clean. |
...
| robustness / statistical testing | `economics.stat.testing.multcomp` (Multiple Testing Pack v1: ONLY Holm FWER + Benjamini-Hochberg FDR, definition-compatible across Python statsmodels and R `stats::p.adjust`), `economics.robustness.*` | **E1** `src-statsmodels`/Holm(1979)/B&H(1995); **E2** `case-dime-niger-asp`(MHT). `multcomp.python.statsmodels` & `multcomp.r.base` are `tested` (NOT verified) for the exact Holm/BH-on-frozen-Grunfeld scope only; family definition is an explicit Director/user decision, NOT machine-inferred; adjusted p-values do NOT establish scientific validity; NOT generic multiple-testing coverage. |
...
#### Causal IV/2SLS Pack v1 — narrow scope

- **IV/2SLS** (`economics.causal.iv`) is a **narrow** linear IV/2SLS estimator with auditable diagnostics. It explicitly distinguishes the structural equation / outcome, endogenous regressor(s), excluded instrument(s), included exogenous controls, fixed effects (if exercised), clustering/covariance convention, sample, first stage, reduced form (where available), and the second-stage 2SLS estimate. It does **NOT** machine-verify the exclusion restriction and does **NOT** interpret relevance diagnostics as proof of instrument validity — those are human/Director scientific judgments.
- **Director decisions** required at minimum: instrument definition, endogenous-regressor definition, exclusion-restriction argument, clustering level, sample exclusions, and model/specification choice.
- **Diagnostic semantics**: conventional first-stage F, partial F, Kleibergen–Paap rk Wald F, Cragg–Donald Wald F, Sanderson–Windmeijer conditional F, Anderson–Rubin / Stock–Wright weak-IV-robust inference, and underidentification (Anderson canonical corr. LM) each have distinct definitions and are produced by different engines. Only definition-compatible quantities are compared across engines; tool-specific statistics are documented, never forced to equality or fabricated.
- **Maturity (honest)**: `causal.iv.python.linearmodels` and `causal.iv.stata.ivreg2` are both promoted to `tested` for the exact demonstrated scope (homoskedastic 2SLS on the Card (1993) NLSYM returns-to-schooling benchmark, N=3010, cross-engine coef/SE/N match) — NOT `verified`. No IV implementation is `verified`; the tested claim is the 2SLS coefficient/SE/N (and captured diagnostics), not general IV validity.
