#!/usr/bin/env node
// P7 panel_fe benchmark runner wrapper (Stata reghdfe) - parameterized.
...
export function buildStataResultFromRaw(rawText, man, benchmarkId = "grunfeld_twfe_cluster") {
  const kv = {};
  ...
  const infCfg = {
    ...
    stata_dof_evidence: { df_m: kv.e_df_m, df_r: kv.e_df_r, df_a: kv.e_df_a, df_a_nested: kv.e_df_a_nested, dofmethod: kv.e_dofmethod, vce: kv.e_vce, clustvar: kv.e_clustvar },
    note: "firm FE nested in cluster(firm) redundant for DoF; cluster small-sample correction applied",
  };
  return {
    ...
    native_default: { cov_type: "reghdfe_default", coefficients: { value: num("b_value"), capital: num("b_capital") }, std_errors: { value: num("default_se_value"), capital: num("default_se_capital") }, n: num("n") },
  };
}
...
if (isMain) {
  ...
  const raw = arg("raw", join(root, "domains/economics/benchmarks/panel_fe/results/stata_raw.txt"));
  const out = arg("out", join(root, "domains/economics/benchmarks/panel_fe/results/stata.json"));
  const manPath = arg("manifest", join(root, "domains/economics/benchmarks/panel_fe/benchmark.grunfeld.json"));
  const benchId = arg("benchmark-id", "grunfeld_twfe_cluster");
  ...
  const result = buildStataResultFromRaw(readFileSync(raw, "utf8"), man, benchId);
