...
// C. upstream change propagation
const dC = join(TMP, "c"); copyValid(dC); writeManifest(dC, manifestHash(dC));
const c0 = render(dC); ok("C old manifest renders initially", c0.ok === true, `ok=${c0.ok}`);
const ep = join(dC, "estimates.json"); const eo = JSON.parse(readFileSync(ep, "utf8")); eo.estimates[0].estimate = 9.99; writeFileSync(ep, JSON.stringify(eo, null, 2) + "\n", "utf8");
const cOld = render(dC);
ok("C stale presentation hash blocks render", cOld.ok === false && cOld.errors.some((e) => e.includes("hash 不匹配")), `ok=${cOld.ok} errs=${JSON.stringify(cOld.errors)}`);
writeManifest(dC, manifestHash(dC));
const cNew = render(dC);
ok("C refreshed binding rerenders and output changed with upstream", cNew.ok === true && cNew.output.includes("9.9900") && cNew.output !== c0.output, `ok=${cNew.ok} changed=${cNew.output !== c0.output}`);
