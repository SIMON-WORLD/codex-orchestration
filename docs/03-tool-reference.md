...
- **会话/任务编排（主导者派发核心）**（9）：`create_thread`、`fork_thread`、`get_handoff_status`、`handoff_thread`、`list_archived_threads`、`list_threads`、`read_thread`、`send_message_to_thread`、`wait_threads`
- **会话组织与展示**（6）：`navigate_to_codex_page`、`open_in_codex`、`set_thread_archived`、`set_thread_pinned`、`set_thread_title`、`share_thread`
- **定时/自动化/托管**（1）：`automation_update`
- **应用/运行时/杂项**（7）：`capture_screen_context`、`consume_usage_reset`、`end_realtime_voice_call`、`list_projects`、`load_workspace_dependencies`、`read_thread_terminal`、`uninstall_plugin`
...
| `handoff_thread` | 将会话（含 git 状态）移交到本机/远程或 worktree。 | 把会话（含 git 状态）移交到本机/远程或 worktree 继续。 | `handoff_thread({threadId:'...', destinationHostId:'local'})` |
...
| `send_message_to_thread` | 向已有会话发送后续消息（派活/补充/纠偏）。 | 给某条会话补派活、补充要求或纠偏。 | `send_message_to_thread({threadId:'...', prompt:'...'})` |
| `wait_threads` | 等待一条或多条会话跑到完成或需关注。 | 并行派了多条会话后，等它们跑完/拿最新状态。 | `wait_threads({targets:[{threadId:'...'}], timeoutMs:120000})` |
...
| `open_in_codex` | 在 Codex 面板展示文件/浏览器/终端/评审。 | 在面板展示文件/浏览器/终端/评审。 | `open_in_codex({target:{type:'file', path:'...'}})` |
...
| 工具 | 作用 | 何时用 | 示例 |
|---|---|---|---|
| `capture_screen_context` | 语音对话时读取当前 Codex 页面状态。 | 仅语音对话时读取当前页面状态；普通文字会话不要用。 | `capture_screen_context()` |
...
| `list_projects` | 列出可用于创建会话的项目。 | create_thread 指定归属项目时先查项目 id。 | `list_projects()` |
| `load_workspace_dependencies` | 定位本机 Node/Python 与常用库路径。 | 做表格/PPT/文档时定位本机运行时与库。 | `load_workspace_dependencies()` |
